 ...
 */
define(["require", "exports", "N/log", "N/search", "N/email", "N/render", "N/file", "../Global/Constants"], function (require, exports, log, search, email, render, file, constants) {
    ...
    function execute(pContext) {
        try {
            ...
            var reportFile = createReportFile(approvalRequests);
            // Send the email
    ...
    function createReportFile(pApprovalRequests) {
        ...
        if (pApprovalRequests.length > 0) {
            ...
            for (var i = 0; i < pApprovalRequests.length; i++) {
                var pendingActionFrom = pApprovalRequests[i][constants.APPROVAL_REQUEST_OBJECT.VENDOR_OR_TOV_SIDE];
                var dateCreated = pApprovalRequests[i][constants.APPROVAL_REQUEST_OBJECT.DATE];
                var vendorName = pApprovalRequests[i][constants.APPROVAL_REQUEST_OBJECT.VENDOR];
                var purchaseOrderDate = pApprovalRequests[i][constants.APPROVAL_REQUEST_OBJECT.PO_DATE];
                var purchaseOrderNumber = pApprovalRequests[i][constants.APPROVAL_REQUEST_OBJECT.PO_TRANID];
