...
export function getHomePage(pPendingApprovalRequestData)
{
...
function getCategoriesQuantities(pPendingApprovalRequestData)
{
    ...
    for (let i = 0; i < pPendingApprovalRequestData.length; i++)
    {
        ...
        let isPartsOrder = pPendingApprovalRequestData[i][constants.APPROVAL_REQUEST.FIELDS.PURCHASE_ORDER_IS_REPLACEMENT];
...
function getSidebarView(pPendingApprovalRequestData)
{
    ...
    let newVendorsReqScriptID = constants.SCRIPTS.NEW_VENDOR_REQUESTS.ID;
    let newVendorsReqDeployID = constants.SCRIPTS.NEW_VENDOR_REQUESTS.DEPLOY;
    let newVendorsReqURL = functions.getSuiteletURL(newVendorsReqScriptID,newVendorsReqDeployID, false);
...
export function getPendingApprovalRequestsView(pPendingApprovalRequestData, pPageID)
{
...
function getOrdersListView(pPendingApprovalRequestData, pPageID, pFilters)
{
    ...
    for (let i = 0; i < pPendingApprovalRequestData.length; i++)
    {
        ...
        if (processedApprovalRequests.indexOf(approvalRequestID) === -1 && !loadPlanCondition)
        {
            ...
            let link = `${functions.getCurrentSuiteletURL(false)}&id=${approvalRequestID}&po=${purchaseOrderID}&page=${pPageID}`;
            ...
            let total = pPendingApprovalRequestData[i][constants.APPROVAL_REQUEST.FIELDS.TOTAL] ? pPendingApprovalRequestData[i][constants.APPROVAL_REQUEST.FIELDS.TOTAL] : '';
...
function getPartsOrdersListView(pPendingApprovalRequestData, pPageID)
{
    ...
    for (let i = 0; i < pPendingApprovalRequestData.length; i++)
    {
        ...
        let vendorOrTOVSide = pPendingApprovalRequestData[i][constants.APPROVAL_REQUEST.FIELDS.VENDOR_OR_TOV_SIDE];
        let isPartsOrder = pPendingApprovalRequestData[i][constants.APPROVAL_REQUEST.FIELDS.PURCHASE_ORDER_IS_REPLACEMENT];
        let isnShipped = pPendingApprovalRequestData[i][constants.APPROVAL_REQUEST.FIELDS.ISN_SHIPPED];
        ...
        if (processedApprovalRequests.indexOf(approvalRequestID) === -1)
        {
            ...
            if (isPartsOrder)
            {
                let partsShipMethod = pPendingApprovalRequestData[i][constants.APPROVAL_REQUEST.FIELDS.PURCHASE_ORDER_PARTS_SHIP_METHOD] ? pPendingApprovalRequestData[i][constants.APPROVAL_REQUEST.FIELDS.PURCHASE_ORDER_PARTS_SHIP_METHOD] : '';
...
function getShipmentsListView(pPendingApprovalRequestData, pPageID, pFilters) {
    ...
    for (let i = 0; i < pPendingApprovalRequestData.length; i++) {
        ...
        if (loadPlanCondition && inboundShipmentID && processedInboundShipments.indexOf(inboundShipmentID) === -1) {
            ...
            let confirmedDepartureDate = pPendingApprovalRequestData[i][constants.INBOUND_SHIPMENT_OBJECT.CONFIRMED_DEPARTURE_DATE];
            ...
            let vendor = pPendingApprovalRequestData[i][constants.APPROVAL_REQUEST.FIELDS.VENDOR];
...
export function getPurchaseOrderView(pPendingApprovalRequestData, pApprovalRequestData, pPurchaseOrderData, pApprovalRequestCommentsData, pPageID)
{
    let approvalID = pApprovalRequestData[constants.APPROVAL_REQUEST.FIELDS.INTERNALID];
...
// Get the view of a specific Purchase Order
export function getInboundShipmentView(pPendingApprovalRequestData, pApprovalRequestData, pInboundShipmentData, pApprovalRequestCommentsData, pPageID)
{
    let requestApproved = pApprovalRequestData[constants.APPROVAL_REQUEST.FIELDS.APPROVED];
    let vendorOrTOVSide = pApprovalRequestData[constants.APPROVAL_REQUEST.FIELDS.VENDOR_OR_TOV_SIDE];
    let PIFileUploaded = pApprovalRequestData[constants.APPROVAL_REQUEST.FIELDS.PI_FILE_UPLOADED];
    ...
    for (let i = 0; i < pData.length; i++)
    {
        ...
        <tr class="item-line">
            ...
            <td class="line-action">
                ...
                <input type="checkbox" class="approve-check-input" id="approve-line-check-${i}" style="display: none;" ${status == "Approved" ? "checked" : ""}>
...
function getRelatedInboundView(pArgs){
    ...
    for (let i = 0; i < orders.length; i++) {
        ...
        let link = `${functions.getCurrentSuiteletURL(false)}&id=${pArgs.approvalID}&po=${purchaseOrderID}&isn=${inboundShipmentID}&page=${pArgs.pageID}`;
